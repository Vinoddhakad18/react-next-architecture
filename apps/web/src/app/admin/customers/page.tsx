...
export default function CustomersManagementPage() {
  ...
  const [formErrors, setFormErrors] = useState<Partial<Record<keyof UpdateCustomerRequest, string>>>({});
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setItems(normalized as Customer[]);
        setPagination(
          payload.meta ?? {
            page: params.page || 1,
  ...
  const validateForm = () => {
    const errors: Partial<Record<keyof UpdateCustomerRequest, string>> = {};
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = isEditMode && editingItem
        ? await customerService.updateCustomer(editingItem.id as string, payload)
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      const response = await customerService.deleteCustomer(itemToDelete.id as string);
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">View, create, update, and delete customers.</p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <Button onClick={handleOpenCreateModal} variant="primary">Add Customer</Button>
        </div>
      </div>

      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div className="text-sm font-medium text-slate-700">Customer list</div>
        ...
        {isLoading ? (
          ...
          <div className="px-6 py-12 text-center text-sm text-slate-500">No customers found.</div>
        ) : (
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm text-slate-600">
            ...
            <span className="font-semibold text-slate-900">{pagination.total}</span> customers
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={isEditMode ? 'Edit Customer' : 'Add Customer'} size="md">
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={handleCloseDeleteModal} title={'Delete Customer'} size="sm">
