/**
 * Menus Active List API Route
 * Handles fetching active menus list
 */
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorData = { message: errorText || 'Failed to fetch active menus' };
      }

      return NextResponse.json(
        {
          success: false,
          message: errorData.message || 'Failed to fetch active menus',
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'Active menus fetched successfully',
        data: data.data || data,
      },
      { status: 200 }
    );
